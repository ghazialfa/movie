...
import type { PaginatedResponse, Movie, MovieDetail, GenreList, AiMoviesResponse, FavoriteItem } from "@/types/movies";

export async function getMovies({ search, page = 1, token }: { search?: string; page?: number; token: string }) {
...
export async function getMoviesByGenre({ id, page = 1, token }: { id: number; page?: number; token: string }) {
...
export async function getAiMovies({ userRequest, token }: { userRequest: string; token: string }) {
  const { data } = await api.post<AiMoviesResponse>("/movies/ai", { userRequest }, {
    headers: { Authorization: `Bearer ${token}` },
  });
...
export async function updateFavoriteStatus({ id, status, token }: { id: number; status: string; token: string }) {
  const { data } = await api.patch(`/favorites/${id}`, { status }, {
    headers: { Authorization: `Bearer ${token}` },
  });
  return data;
}
...
export default function Navbar() {
	...
	const [clientUserId, setClientUserId] = React.useState<string | null>(null);
	React.useEffect(() => {
	...
	const token = storeToken || clientToken || "";
	const userId = storeUserId || clientUserId || "";
	...
	return (
		<header className="flex items-center justify-between bg-gray-200 px-4 py-3 text-gray-900 md:px-6 dark:bg-gray-900 dark:text-white">
							...
							<ul className="grid w-[300px] gap-3 p-4 md:w-[400px] md:grid-cols-3 lg:w-[400px]">
								{(data?.genres || []).map(({ id, name }) => (
									<div key={id} className="mt-2">
										<Button asChild size="sm" className="w-full">
											<Link href={`/movie/genres/${slugify(name)}`}>
												{name}
											</Link>
										</Button>
									</div>
            ...
            <NavigationMenuItem>
              <NavigationMenuLink asChild className={navigationMenuTriggerStyle()}>
                <Link href={userId ? `/user/${userId}/favorites` : "/movie"}>
                  Favorites
                </Link>
              </NavigationMenuLink>
            </NavigationMenuItem>
			...
			<div className="flex items-center gap-2">
				...
				<Button onClick={handleLogout} className="rounded-md bg-red-300" variant="outline">
					Logout
				</Button>
			</div>
		</header>

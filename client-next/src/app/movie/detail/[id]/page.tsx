      ...
      <div className="container px-4 md:px-6">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          ...
          <div className="md:col-span-2 space-y-4">
            ...
            <div className="text-sm text-gray-500">{data.release_date} • {data.original_language?.toUpperCase()}</div>
            <div className="flex flex-wrap gap-2">
              {(data.genres || []).map((g) => (
                <span key={g.id} className="text-xs px-2 py-1 rounded bg-gray-200">{g.name}</span>
              ))}
            </div>
            <p className="text-gray-700">{data.overview}</p>
            <div className="text-sm text-gray-600">Rating: {data.vote_average} ({data.vote_count} votes)</div>
          </div>
        </div>
      </div>
    </section>

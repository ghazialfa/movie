...
export default function GenreDetailPage() {
  ...
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, isPending } = useInfiniteQuery<PaginatedResponse<Movie>>({
    queryKey: ["movies-genre", genreId, token],
    queryFn: ({ pageParam = 1 }) => getMoviesByGenre({ id: Number(genreId), page: Number(pageParam), token: token || "" }),
  ...
  return (
    <section className="w-full py-12 md:py-24 lg:py-32">
      <div className="container px-4 md:px-6">
        <h2 className="text-2xl font-bold tracking-tighter sm:text-3xl md:text-4xl mb-6">Genre Movies</h2>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 md:gap-6">
